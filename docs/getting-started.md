...
Once the app is installed on your repository, you now need to select a mode of
operation. Today, there are two modes of operation to use the app. In the "lite"
mode, the app is installed without configuring gittuf policy for the repository.
The app records attestations for pull request approvals and merges, and can be
used to generate source provenance attestations for the upcoming SLSA source
track. The lite mode is recommended if you're interested in slowly ramping up
with gittuf use.
...
If you would like to use the app in Lite mode, then you don't need to do
anything further to configure it. If you'd like to use the app in Full mode,
however, you'll need to configure gittuf on the repository, as described below.
